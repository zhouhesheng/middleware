...
def _test_xxx_snapshot_query_filter_dataset(
    dataset_name,
    properties_list,
    ...
    with dataset(pool_name, dataset_name) as dataset_config:
        ...
        with snapshot(dataset_id, "snap01") as snap01_config:
            ...
            results = GET("/zfs/snapshot", payload)
            ...
            with snapshot(dataset_id, "snap02") as snap02_config:
                ...
                with dataset(pool_name, f"{dataset_name}2") as dataset2_config:
                    with snapshot(dataset2_config['id'], "snap03") as snap03_config:
                        ...
                        payload.update({'query-filters': [['dataset', '=', dataset2_config['name']]]})
                        results = GET("/zfs/snapshot", payload)
                        assert results.status_code == 200, results.text
                        assert isinstance(results.json(), list), results.text
                        snaps = results.json()
                        ...
                        payload.update({'query-filters': [['dataset', '=', f"{dataset_name}-BOGUS"]]})
                        results = GET("/zfs/snapshot", payload)
                        assert results.status_code == 200, results.text
                        assert isinstance(results.json(), list), results.text
                        snaps = results.json()
...
def _test_simple_snapshot_query_filter_dataset(dataset_name, properties_list):
    ...
    :param properties_list: a list of strings, the names to be queried in snapshot properties option
    """
    _test_xxx_snapshot_query_filter_dataset(
        dataset_name,
        properties_list,
        expected_keys=['pool', 'name', 'type', 'snapshot_name', 'dataset', 'id', 'createtxg'],
        unexpected_keys=['properties']
...
def _test_simple_snapshot_query_filter_snapshot(dataset_name, properties_list):
    ...
    :param properties_list: a list of strings, the names to be queried in snapshot properties option
    """
    _test_xxx_snapshot_query_filter_snapshot(
        dataset_name,
        properties_list,
        expected_keys=['pool', 'name', 'type', 'snapshot_name', 'dataset', 'id', 'createtxg'],
        unexpected_keys=['properties']
...
def _test_simple_snapshot_query_filter_pool(dataset_name, properties_list):
    ...
    :param properties_list: a list of strings, the names to be queried in snapshot properties option
    """
    _test_xxx_snapshot_query_filter_pool(
        dataset_name,
        properties_list,
        expected_keys=['pool', 'name', 'type', 'snapshot_name', 'dataset', 'id', 'createtxg'],
        unexpected_keys=['properties']
